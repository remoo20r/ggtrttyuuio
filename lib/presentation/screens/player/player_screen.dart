...
class _PlayerScreenState extends ConsumerState<PlayerScreen> {
  ...
  void initState() {
    ...
    // Android TV performance: force libmpv to use the device's hardware video
    // decoder (mediacodec) instead of falling back to slow software decoding —
    // software decoding is what makes playback run like slow-motion on 2 GB
    // boxes. Also relax audio so a stream whose audio codec mpv can't decode
    // still plays (fixes "error decoding audio"), and tune live buffering for
    // fast, low-latency channel starts.
    if (Platform.isAndroid && native is NativePlayer) {
      unawaited(() async {
        try {
          // Hardware decode with automatic software fallback only if strictly
          // needed ('mediacodec-copy' is the most compatible HW path).
          await native.setProperty('hwdec', 'mediacodec-copy');
          // Let mpv fall back gracefully instead of erroring out on an
          // unsupported audio/video codec.
          await native.setProperty('hwdec-codecs', 'all');
          // Live streams: keep latency low and don't hoard RAM.
          await native.setProperty('cache', 'yes');
          await native.setProperty('demuxer-max-bytes', '8MiB');
          await native.setProperty('demuxer-max-back-bytes', '4MiB');
          // Smoother frame pacing on TV panels.
          await native.setProperty('video-sync', 'audio');
          // Don't abort playback if one audio frame fails to decode.
  ...
  void _open(String url) {
    ...
    // Resume reliably: hand the start offset straight to the media so playback
    // begins at the saved point, instead of relying only on a post-load seek
    // (which some VOD streams ignored, restarting the movie from zero). The
    // duration-stream seek below stays as a fallback for players/streams that
    // don't honour `start`.
    final resume = _pendingResumeMs;
    final media = (resume != null && resume > 5000)
        ? Media(url, start: Duration(milliseconds: resume))
        : Media(url);
    // If `start` is handling the resume, drop the pending seek so the
    // duration-stream fallback doesn't fire a redundant second seek.
    if (resume != null && resume > 5000) _pendingResumeMs = null;
    // `open` with play=true tears down the previous stream cleanly first.
    // Skipping this caused a black screen (no audio/video) when zapping between
    // live channels, because the old stream's decoder wasn't released before
